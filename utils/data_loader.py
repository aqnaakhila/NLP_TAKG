...
def load_vocab(opt):
    ...
    if not opt.custom_vocab_filename_suffix:
        word2idx, idx2word, vocab, bow_dictionary = torch.load(opt.vocab + '/vocab.pt', weights_only=False)
    else:
        word2idx, idx2word, vocab = torch.load(opt.vocab + '/vocab.%s.pt' % opt.vocab_filename_suffix, weights_only=True)
    # assign vocab to opt
